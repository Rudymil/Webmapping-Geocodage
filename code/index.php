...
<html lang="fr">
	...
	<head>
		...
		<style>
			...
			table.tbl_bdd,th{ /* style des entetes des tableaux */
		...
		<script>
			...
			function initialize(){ // lors du chargement de la page
				var latlng = new google.maps.LatLng(48.849335,2.607764); // coordonnees a charger
				var mapOptions = {
					zoom:14, // zoom a charger
					center:latlng
				}
				map = new google.maps.Map(document.getElementById("map"),mapOptions); // carte Google
				// affichage de toutes les coordonnees
				for(var i=1;i<=nb_ligne_geo;i++){ // parcours des geocodages
					var loc = { lat: parseFloat(row_geo[i][3]), lng: parseFloat(row_geo[i][4]) }; // { lat: 48.843336, lng: 2.622286 }; // parseFloat() => converti en flotant
					console.log(loc); // affiche la localisation
					for(var j=1;j<=nb_ligne_adr;j++){ // cherche l adresse correspondante
						//console.log(row_geo[i][1]);
						//console.log(row_adr[j][1]);
						if(row_geo[i][1]==row_adr[j][1]){ // s il s agit de l adresse
							var bulle = row_adr[j][2]+" "+row_adr[j][3]+", "+row_adr[j][4]+" "+row_adr[j][5]+", "+row_adr[j][6]; // numero voierie code_postal commune pays
							console.log(bulle); // affiche l info bulle
							break; // si tout est bon
						}
					}
	...
	<!-- BODY -->
	<body onload='javascript:initialize()'> <!-- appelle a l initialisation -->
		<h2 onclick='javascript:afficher();'><center>Comparateur de web services de géocodage - la consultation<center></h2> <!-- titre -->
		...
		<script type="text/javascript">
			// VARIABLE PHP
			var nb_ligne_adr = <?php echo $nb_ligne_adr; ?>; // nombre de lignes dans la table adresse
			console.log("Nombre de ligne du tableau des adresses : " + nb_ligne_adr);
			var row_adr = <?php echo json_encode($row_adr); ?>; // tableau des adresses
			console.log(row_adr);
			...
			function addMarker(location,map,bulle){ // Adds a marker to the map.
				...
				marker.addListener('click',function(){ // affiche l info bulle au clic
					infowindows.open(map,marker); // ouvrir l info bulle
				})
			}
		</script>
	</body>
